...
The hook-core line has no custom kernel driver. DevWT assigns each registered worktree a deterministic localhost port range, then uses a user-mode Win32 hook runtime to rewrite:
...
- No custom DevWT kernel driver.
- No test-signing requirement.
...
Then open **PowerShell as Administrator** in `private\installer\DevWT` and run:
...
devwt context describe "PR 22558 review"
...
Private VM notes, smoke outputs, and installer bundles belong under `private/`, which is ignored.

...
if ($KillHookedApplications -and -not $UpdateHookRuntime) {
    throw "KillHookedApplications requires -UpdateHookRuntime."
}
...
function Stop-DevwtHookedApplications([switch] $Kill) {
    ...
    if (-not $Kill) {
        Write-Warning "Found applications with DevWT hook DLL loaded. KillHookedApplications was not specified; they were not terminated."
    ...
    foreach ($application in $applications) {
        Write-DevwtUpdateLog "Terminating DevWT-hooked application $($application.Name) PID $($application.Id)."
    ...
    throw "DevWT-hooked application process(es) could not be terminated: $(($remaining.Id | Sort-Object) -join ', ')."
}
...
if ($UpdateHookRuntime) {
    if ($KillHookedApplications) {
        Write-Host "DevWT managed service and hook runtime updated after terminating hooked applications."
    }
    else {
...
if ($UpdateHookRuntime) {
    if ($KillHookedApplications) {
        Write-Host "Terminated applications are not restarted automatically."
    }
    else {

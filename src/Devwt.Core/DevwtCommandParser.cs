...
public sealed record UpdateCommand(bool StopRunningApplications) : DevwtCommand;
...
public static class DevwtCommandParser
{
    public const string HelpText = """
        devwt commands:
          ...
          update [--stop-running-applications]                      # default keeps hooked applications running
          status
          ui
    ...
    public static DevwtCommand Parse(IReadOnlyList<string> args, string? currentDirectory = null)
    {
        ...
        return args[0].ToLowerInvariant() switch
        {
            ...
            "ui" => new UiCommand(),
            "update" => ParseUpdate(args.Skip(1).ToArray()),
    ...
    private static DevwtCommand ParseUpdate(IReadOnlyList<string> args)
    {
        var stopRunningApplications = false;
        foreach (var option in args)
        {
            if (option.Equals("--stop-running-applications", StringComparison.OrdinalIgnoreCase))
            {
                stopRunningApplications = true;
                continue;
            }

            throw new ArgumentException($"Unknown update option: {option}");
        }

        return new UpdateCommand(stopRunningApplications);
    }

...
Download the public bootstrap script, review it, and run it:
...
The script includes preview releases when selecting the newest published
version. It requires exactly one versioned installer ZIP and its matching
`.sha256` release asset, verifies the archive, and only then launches the
bundled installer. Use `-DownloadOnly -Destination <folder>` to stop after
downloading and verifying the ZIP.
...
Install from the generated `artifacts\installer\DevWT` folder in an elevated PowerShell:

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Install-DevWT.ps1
```

## Update an installed DevWT

Download the newest published release, verify its checksum, stage the new
runtime, and preserve applications that are already running:

```powershell
devwt update
```

The managed service briefly restarts, but hooked application processes keep
running with the runtime they already loaded. New processes use the newly staged
runtime.

To terminate hooked applications before switching the runtime:

```powershell
devwt update --stop-running-applications
```

This mode does not restart the terminated applications automatically.

...
owns the original endpoint. DevWT does not need a kernel driver, TESTSIGNING,
or loopback aliases for the default path.
...
as Rider, ABP Studio, or Store-installed apps such as Codex.

The folder watcher does not use a custom driver. It performs an initial process
scan, then tries to subscribe to Windows kernel process-start events from the
service process. If that event session is unavailable, it falls back to the

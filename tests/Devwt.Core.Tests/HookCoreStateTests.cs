...
public sealed class HookCoreStateTests
{
    ...
    public void Git_worktree_porcelain_parser_reads_worktree_roots_and_refs()
    {
        var worktrees = GitWorktreeParser.ParsePorcelain(
            ...
            worktree C:/work/sample-feature
        ...
        Assert.Collection(
            ...
            item =>
            {
                Assert.Equal(@"C:\repos\sample", item.RootPath);
                Assert.Equal("main", item.RefName);
            },
            item =>
            {
                Assert.Equal(@"C:\work\sample-feature", item.RootPath);
    ...
    public void Hook_installer_calls_internal_worktree_ready_command_and_is_idempotent()
    {
        ...
        var first = PostCheckoutHookInstaller.Install(hookPath, "devwt", "repo-sample");
        var second = PostCheckoutHookInstaller.Install(hookPath, "devwt", "repo-sample");
        ...
        Assert.Contains("devwt hook worktree-ready --repo-id \"repo-sample\" --path \"$PWD\"", hook);
    ...
    public void Hook_installer_replaces_existing_devwt_block_on_upgrade()
    {
        ...
        var result = PostCheckoutHookInstaller.Install(hookPath, "devwt", "repo-sample");
        var hook = File.ReadAllText(hookPath);
    ...
    public void State_store_round_trips_repositories_contexts_and_routing()
    {
        ...
        var repository = new DevwtRepository(
            ...
            GitCommonDir: @"C:\repos\sample\.git",
            LinkedRepositories: [new LinkedRepository("shared", "../shared", @"C:\repos\shared")]);
        ...
        store.SaveRouting(new DevwtRoutingState(
            ExplicitLinkMaps: [new DevwtLinkMap("shared", @"C:\work\sample-feature", @"C:\work\shared-feature")],
            ActiveTarget: new DevwtActiveTarget("ctx-sample-feature", 5025, "http"),
            ProcessTargets: [new DevwtProcessTarget(1234, "ctx-sample-feature")],
            ApplicationTargets: [new DevwtApplicationTarget(@"C:\tools\chrome.exe", "ctx-sample-feature", 5025, "auto")]));
        ...
        Assert.Equal("Review authentication changes", loadedContext.Description);
        ...
        Assert.Equal(
            new DevwtPortActiveTarget("ctx-sample-feature", 5025),
        ...
        Assert.Equal("ctx-sample-feature", processTarget.ContextId);
        ...
        Assert.Equal("ctx-sample-feature", applicationTarget.ContextId);

...
public sealed class HookCoreServiceTests
{
    ...
    public void Gateway_worker_alias_contains_backend_image_names_and_a_stable_endpoint_hash()
    {
        var first = DevwtGatewayWorkerNames.BuildAliasFileName(
            ["SampleApp.HttpApi.Host.exe", "vite.exe"],
            "127.0.0.1:44334|SampleApp.HttpApi.Host.exe|vite.exe");
        var second = DevwtGatewayWorkerNames.BuildAliasFileName(
            ["SampleApp.HttpApi.Host.exe", "vite.exe"],
            "127.0.0.1:5001|SampleApp.HttpApi.Host.exe|vite.exe");

        Assert.Contains("SampleApp.HttpApi.Host", first, StringComparison.OrdinalIgnoreCase);
    ...
    public void Gateway_workers_read_the_supervisors_atomic_route_snapshot()
    {
        ...
        {
            Description = "Review gateway routing"
        };
        ...
        Assert.Equal("Review gateway routing", loaded.DescriptionForContext("ctx-a"));
    }
    ...
    public void Linked_repo_routing_prefers_linked_target_before_self_context()
    {
        var callerRepo = new DevwtRepository(
            ...
            @"C:\repos\sample",
            @"C:\repos\sample\.git",
            [new LinkedRepository("shared", "../shared", @"C:\repos\shared")]);
        var calleeRepo = new DevwtRepository("repo-shared", "shared", @"C:\repos\shared", @"C:\repos\shared\.git", []);
        var caller = Context("ctx-sample-feature", "repo-sample", @"C:\work\sample", "127.80.1.10");
        var self = new GatewayRoute("ctx-sample-feature", "repo-sample", @"C:\work\sample", 5025, "127.80.1.10", 5025, 10);
        var linked = new GatewayRoute("ctx-shared-feature", "repo-shared", @"C:\work\shared", 5025, "127.80.1.11", 5025, 11);
        var table = GatewayRouteTable.FromRoutes(
            [self, linked],
            new DevwtRepositoryState([callerRepo, calleeRepo]),
            new DevwtContextState([caller, Context("ctx-shared-feature", "repo-shared", @"C:\work\shared", "127.80.1.11")]),
            DevwtRoutingState.Empty);

        var route = table.Resolve(5025, callerContextId: "ctx-sample-feature", requestContextId: null, cookieContextId: null, includeActiveTarget: true);

        Assert.NotNull(route);
        Assert.Equal("ctx-shared-feature", route.ContextId);
    }
    ...
    public void Web_ui_action_mapper_builds_management_requests()
    {
        var addRepository = DevwtWebUiActionMapper.MapManagement(new DevwtWebUiAction(
            ...
            LinkedRepositories: [new LinkedRepositoryInput("shared-lib", @"D:\repos\shared-lib")]));
        var pauseRepository = DevwtWebUiActionMapper.MapManagement(new DevwtWebUiAction(
            "pause-repository",
            RepositoryName: "repo-sample"));
        ...
        Assert.Equal(@"D:\repos\sample-app", addRepository.AddRepository?.WorkingDirectory);
        Assert.Equal("sample-app", addRepository.AddRepository?.Name);
        Assert.Equal(
            new LinkedRepositoryInput("shared-lib", @"D:\repos\shared-lib"),
        ...
        Assert.Equal("repo-sample", pauseRepository.RepositoryName);
        Assert.Equal(DevwtControlOperation.Resume, resumeRepository.Operation);
        Assert.Equal("repo-sample", resumeRepository.RepositoryName);
        Assert.Equal(
            new DevwtPortQuery(44334, @"D:\repos\sample-app", "ctx-sample"),
            portCheck.PortQuery);
        Assert.Equal(
            new DevwtIdeWatch("Rider", ImagePath: @"C:\Tools\rider64.exe"),
            ideWatch.IdeWatch);
        Assert.Equal(
            new DevwtProxyChildTarget("ctx-sample", 44334, GatewayRouteProtocol.Tcp, Force: true),
    ...
    public void Control_handler_sets_browser_missing_port_fallback()
    {
        ...
        var handler = new DevwtControlHandler(
            new DevwtManager(
                store,
                new FakeGitInspector(new GitRepositoryInfo(@"C:\work\sample", @"C:\work\sample\.git", [])),
                new RecordingHookRuntimeConfigurator()),
            store);
    ...
    public void Port_queries_find_live_backend_processes_in_auto_or_explicit_context()
    {
        ...
        store.SaveRepositories(new DevwtRepositoryState([repository]));
        store.SaveContexts(new DevwtContextState([
            Context("ctx-a", repository.Id, @"C:\work\a", "127.0.0.1") with { Description = "Gateway routing review" },
            Context("ctx-b", repository.Id, @"C:\work\b", "127.0.0.1")
        ]));
        ...
        Assert.Contains("\"Gateway routing review\" (ctx-a)", process.Output, StringComparison.Ordinal);
    ...
    public async Task Gateway_session_rule_routes_browser_process_to_context_started_by_same_application_session()
    {
        ...
        store.SaveRuntimeSettings(new DevwtRuntimeSettings([], [
            new DevwtSessionRule(
                Name: "Sample IDE",
                Match: new DevwtSessionMatch(ProcessName: "sample-ide"),
                Identity: new DevwtSessionIdentity(
        ...
        var processSource = new FixedProcessObservationSource([
            new ProcessObservation(100, null, @"C:\Tools\SampleIDE\sample-ide.exe", null, null, StartTime: "2026-07-13T10:00:00Z"),
            new ProcessObservation(200, 100, @"D:\repos\sample-app\SampleApp.AuthServer.exe", null, null, StartTime: "2026-07-13T10:00:10Z"),
            new ProcessObservation(201, 100, @"D:\repos\sample-app\SampleApp.HttpApi.Host.exe", null, null, StartTime: "2026-07-13T10:00:15Z"),
            ...
            new ProcessObservation(400, null, @"D:\repos\other-app\SampleApp.AuthServer.exe", null, null, StartTime: "2026-07-13T10:00:30Z")
        ]);
        ...
        Assert.Equal("sample-ide:100:2026-07-13T10:00:00Z", entry.SessionId);
    }
    ...
    public void Process_session_resolver_preserves_environment_only_identity_for_codex_started_helpers()
    {
        ...
        var processes = new[]
        {
            new ProcessObservation(
                10,
                null,
                @"C:\Users\developer\AppData\Local\Programs\Ollama\ollama.exe",
                null,
                null,
    ...
    public async Task Gateway_adds_context_response_header_for_http_responses()
    {
        ...
        Assert.Contains("X-DevWT-Description: Review gateway routing\r\n", response, StringComparison.Ordinal);
    ...
    {
        var route = new GatewayRoute("ctx-sample-feature", "repo-sample", @"C:\work\sample", 5025, "127.80.1.10", 5025, 10);
        var table = GatewayRouteTable.FromRoutes(
            [route],
            DevwtRepositoryState.Empty,
            DevwtContextState.Empty,
    ...
    public void Route_snapshot_uses_port_binding_map_to_expose_original_localhost_port()
    {
        using var temp = new TempDirectory();
        var store = new DevwtStateStore(temp.Path);
        var repository = new DevwtRepository("repo-sample", "sample", @"C:\repos\sample", @"C:\repos\sample\.git", []);
        var context = new DevwtContext("ctx-sample", repository.Id, "sample", @"C:\repos\sample", "main", "127.0.0.1", "DevWT-runtime", DevwtContextStatus.Active, AssignedPortBase: 24000);
        store.SaveRepositories(new DevwtRepositoryState([repository]));
        store.SaveContexts(new DevwtContextState([context]));
        File.WriteAllLines(HookPortBindingMap.ResolvePath(temp.Path), [
            "ctx-sample\t44334\t127.0.0.1\t55297\t42"
        ]);
    ...
    public void Route_snapshot_preserves_original_bind_address_for_gateway_listener()
    {
        ...
        store.SaveContexts(new DevwtContextState([context]));
        File.WriteAllLines(HookPortBindingMap.ResolvePath(temp.Path), [
            "ctx-sample\t192.168.1.10\t44334\t192.168.1.10\t55297\t42\ttcp"
        ]);
        var snapshot = new DevwtRouteSnapshotBuilder(
            store,
    ...
    public void Hook_runtime_reconciler_reconfigures_existing_active_contexts()
    {
        ...
        var store = new DevwtStateStore(temp.Path);
        var repository = new DevwtRepository("repo-sample", "sample", @"C:\repos\sample", @"C:\repos\sample\.git", []);
        var active = new DevwtContext("ctx-sample", repository.Id, "sample", @"C:\repos\sample", "main", "127.80.1.10", "DevWT-runtime", DevwtContextStatus.Active);
        var paused = new DevwtContext("ctx-paused", repository.Id, "sample-paused", @"C:\repos\sample-paused", "feature", "127.80.1.11", "DevWT-runtime-paused", DevwtContextStatus.Paused);
        store.SaveRepositories(new DevwtRepositoryState([repository]));
    ...
    public void Repository_pause_and_resume_update_all_and_only_its_contexts()
    {
        ...
        var store = new DevwtStateStore(temp.Path);
        store.SaveRepositories(new DevwtRepositoryState([
            new DevwtRepository("repo-sample", "sample", @"C:\repos\sample", @"C:\repos\sample\.git", []),
            new DevwtRepository("repo-shared", "shared", @"C:\repos\shared", @"C:\repos\shared\.git", [])
        ]));
        store.SaveContexts(new DevwtContextState([
            new DevwtContext("ctx-sample-main", "repo-sample", "sample", @"C:\repos\sample", "main", "127.0.0.1", "DevWT-sample-main", DevwtContextStatus.Active),
            new DevwtContext("ctx-sample-feature", "repo-sample", "sample-feature", @"C:\work\sample-feature", "feature", "127.0.0.1", "DevWT-sample-feature", DevwtContextStatus.Active),
            new DevwtContext("ctx-shared", "repo-shared", "shared", @"C:\repos\shared", "main", "127.0.0.1", "DevWT-shared", DevwtContextStatus.Active)
        ]));
        var manager = new DevwtManager(
            store,
            new FakeGitInspector(new GitRepositoryInfo(@"C:\repos\sample", @"C:\repos\sample\.git", [])),
            new RecordingHookRuntimeConfigurator());

        manager.SetPaused("repo-sample", worktreePath: null, paused: true);

        Assert.All(
            store.LoadContexts().Contexts.Where(context => context.RepositoryId == "repo-sample"),
            context => Assert.Equal(DevwtContextStatus.Paused, context.Status));
        Assert.Equal(
            DevwtContextStatus.Active,
            Assert.Single(store.LoadContexts().Contexts, context => context.RepositoryId == "repo-shared").Status);

        manager.SetPaused("sample", worktreePath: null, paused: false);
    ...
    public void Manager_add_repo_installs_hook_registers_existing_worktrees_and_configures_hook_runtime()
    {
        ...
        var worktreeRoot = Path.Combine(temp.Path, "sample-feature");
        Directory.CreateDirectory(repoRoot);
        Directory.CreateDirectory(worktreeRoot);
        ...
        Assert.Equal("sample", result.Repository.Name);
    ...
    public void Context_description_upserts_by_nested_worktree_path_and_survives_repository_refresh()
    {
        ...
        var manager = new DevwtManager(store, git, new RecordingHookRuntimeConfigurator());
        manager.AddRepository(new AddRepositoryRequest(repoRoot, "sample", []));
        var handler = new DevwtControlHandler(manager, store);

        var first = handler.Handle(new DevwtControlRequest(
            DevwtControlOperation.DescribeContext,
            WorktreePath: nestedPath,
            ContextDescription: "Review gateway routing"));
        var second = handler.Handle(new DevwtControlRequest(
            DevwtControlOperation.DescribeContext,
            WorktreePath: repoRoot,
            ContextDescription: "Review authentication changes"));
        manager.AddRepository(new AddRepositoryRequest(repoRoot, "sample", []));

        Assert.Equal(0, first.ExitCode);
        Assert.Equal(0, second.ExitCode);
        Assert.Equal("Review authentication changes", Assert.Single(store.LoadContexts().Contexts).Description);
        Assert.Equal(
    ...
    public void Context_description_creates_a_missing_context_for_a_registered_repository()
    {
        ...
        Directory.CreateDirectory(repoRoot);
        store.SaveRepositories(new DevwtRepositoryState([
            new DevwtRepository("repo-sample", "sample", repoRoot, gitCommonDir, [])
        ]));
        var git = new FakeGitInspector(
            new GitRepositoryInfo(
                RootPath: repoRoot,
        ...
        var context = manager.SetDescription(repoRoot, "Review gateway routing");

        Assert.Equal("Review gateway routing", context.Description);
        Assert.Equal("Review gateway routing", Assert.Single(store.LoadContexts().Contexts).Description);
    ...
    public void Manager_remove_drops_state_even_when_hook_runtime_cleanup_warns()
    {
        ...
        store.SaveRepositories(new DevwtRepositoryState([repository]));
        store.SaveContexts(new DevwtContextState([
            new DevwtContext("ctx-sample", repository.Id, "sample", @"C:\repos\sample", "main", "127.80.1.10", "DevWT-runtime", DevwtContextStatus.Active)
        ]));
        var manager = new DevwtManager(
            store,
        ...
        var result = manager.RemoveRepository("sample");
    ...
    public void Manager_remove_without_repo_name_drops_only_current_repository()
    {
        using var temp = new TempDirectory();
        var store = new DevwtStateStore(temp.Path);
        var sample = new DevwtRepository("repo-sample", "sample", @"C:\repos\sample", @"C:\repos\sample\.git", []);
        var shared = new DevwtRepository("repo-shared", "shared", @"C:\repos\shared", @"C:\repos\shared\.git", []);
        store.SaveRepositories(new DevwtRepositoryState([sample, shared]));
        store.SaveContexts(new DevwtContextState([
            new DevwtContext("ctx-sample", sample.Id, "sample", @"C:\repos\sample", "main", "127.80.1.10", "DevWT-sample", DevwtContextStatus.Active),
            new DevwtContext("ctx-shared", shared.Id, "shared", @"C:\repos\shared", "main", "127.80.1.11", "DevWT-shared", DevwtContextStatus.Active)
        ]));
        var manager = new DevwtManager(
            store,
            new FakeGitInspector(new GitRepositoryInfo(sample.RootPath, sample.GitCommonDir, [])),
            new RecordingHookRuntimeConfigurator());

        var result = manager.RemoveRepository(repositoryName: null, worktreePath: @"C:\repos\sample\src");

        Assert.Equal(1, result.RemovedRepositories);
        Assert.Equal(1, result.RemovedContexts);
        Assert.Equal(shared.Id, Assert.Single(store.LoadRepositories().Repositories).Id);
        Assert.Equal("ctx-shared", Assert.Single(store.LoadContexts().Contexts).Id);
    }
    ...
    public void Git_inspector_marks_selected_repositories_safe_for_service_owned_git_calls()
    {
        var runner = new RecordingCommandRunner(
            new CommandResult(0, "C:/repos/sample\n"),
            new CommandResult(0, ".git\n"),
            new CommandResult(0, "worktree C:/repos/sample\nHEAD 123\nbranch refs/heads/main\n"),
            new CommandResult(0, "main\n"));
        var inspector = new GitInspector(runner);

        inspector.InspectRepository(@"C:\repos\sample");

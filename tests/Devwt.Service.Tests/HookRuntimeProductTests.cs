...
public sealed class HookRuntimeProductTests
{
    ...
    public void Hook_runtime_planner_maps_active_contexts_to_folder_watcher()
    {
        var context = new DevwtContext(
            ...
            RuntimeName: "DevWT-ctx-volo-feature",
        ...
        Assert.Contains(@"C:\work\volo-feature=ctx-volo-feature,127.0.0.1,127.0.0.1,24000", command.Arguments);
    }
    ...
    public void Hook_port_binding_map_reads_original_and_target_bind_endpoints()
    {
        ...
        File.WriteAllLines(path, [
            "ctx-volo\t192.168.1.10\t44334\t192.168.1.10\t55297\t42\tudp"
        ]);

        var binding = Assert.Single(HookPortBindingMap.Read(path));

        Assert.Equal("ctx-volo", binding.ContextId);
    ...
    public void Managed_updater_preserves_applications_and_can_stage_a_new_hook_runtime()
    {
        ...
        Assert.DoesNotContain("KillHookedApplications", source, StringComparison.Ordinal);
        Assert.DoesNotContain("Stop-DevwtHookedApplications", source, StringComparison.Ordinal);
    ...
    public void Manager_assigns_localhost_and_port_shift_base_without_requiring_aliases()
    {
        ...
        var result = manager.AddRepository(new AddRepositoryRequest(repoRoot, "volo", []));
